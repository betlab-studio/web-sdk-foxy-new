...
>
> 🚨 **CAVEAT WebGL (mobile) — Effekseer/EfkPixi + pré-montage** (réutilisable tous projets) : sur backend **WebGL** (mobile ; desktop = WebGPU, non concerné), `EfkPixi.createContext` fait un `InitWebGL` **synchrone bloquant** qui perturbe l'état GL de pixi. Deux conséquences + leurs fixes :
> 1. **Freeze** si on monte/démonte l'effet à une transition (base↔bonus) → **pré-monter une fois** (init caché sous le loading, cf. ci-dessus) et **toggler une prop `visible`** (skip render, contexte gardé vivant), jamais de remount.
> 2. **1ers sprites corrompus (rect uni)** dessinés juste après l'init — ex. le **press-to-continue** qui apparaît au même instant que le mount à `allAssetsLoaded`. Cause : le restore post-`createContext` ne resettait que `geometry` ; `InitWebGL` perturbe **aussi le binding texture** → les sprites suivants gardent un binding **stale** et samplent une autre texture de l'atlas batch (souvent le logo → couleur du logo). **Fix (dans le package `pixi-effekseer`)** : après `createContext`, invalider **TOUT** le cache GL pixi — `renderer.{geometry,shader,state,texture,buffer,stencil}.resetState()` (le même set que le restore par-frame ; **PAS** `renderer.resetState()` global qui nulle le render target). Diagnostic type : la texture de l'effet est ≠ de la couleur du rect → chercher quel sprite/texture est samplé à tort. Détail complet : `apps/space-mania-front/EFFEKSEER_SPIKE.md` §6.
>
...
> 1bis. **HUD win = compteur INCRÉMENTAL synchro avec la présentation des winGroups** (pas un jump au total). Par défaut l'event « total » (`show-final-win` en base, `setTotalWin` en bonus) pose `winBookEventAmount = total` **immédiatement** — et comme `show-wins` est souvent **fire-and-forget** en base (non-awaited), le HUD saute au total AVANT que les winGroups jouent. Correctif : **la boucle de présentation des wins possède le HUD** et le fait monter **par winGroup**, l'event total ne le pose plus (le retirer pour ce gameType). Process :
>    - Dans `playWinSequence` : `runningWinBook = baseWinBook` au départ ; après l'affichage du montant de **chaque group** → `runningWinBook = min(runningWinBook + group.totalFinalPayout × BOOK_AMOUNT_MULTIPLIER, maxWinBook); stateBet.winBookEventAmount = runningWinBook`. Reconcile à `finalWinBook` à la fin (rounding-safe).
>    - **`baseWinBook`** = point de départ du compteur : **`0` en base/feature** (le HUD compte le gain du spin) ; **`stateBonusWin.totalBeforeCurrentSpin` en freegame** (le HUD compte le **cumul du bonus** qui grandit spin après spin). Vérifier l'ordre des events : le tracker cumul (`show-fs-spin-win`) doit se mettre à jour **APRÈS** `show-wins`, sinon la base est double-comptée.
>    - **Unités** : `group.totalFinalPayout` = multiplicateur de mise (`payout × groupMultiplier`, inclut les mult. bonus/feature) → `× BOOK_AMOUNT_MULTIPLIER` (100) = book units. `Σ groups × 100 == amount` de l'event total (même source) → reconcile exact.
>    - **Retirer le set du HUD dans l'event total** pour le gameType concerné (base/feature : `show-final-win` ; garder le calcul du `displayAmount` pour l'overlay big-win). En bonus, `setTotalWin` peut rester (il reconcile à la MÊME valeur que le compteur → pas de jump).
>    - **SKIP (cf. §GATING §2ter)** : sur abort/skip de la séquence → `stateBet.winBookEventAmount = finalWinBook` (jump au total/cumul) ; l'incrément par-group tourne aussi quand un group est skippé individuellement (le gain compte quand même) → le HUD atteint toujours le total, skip ou non.
...
### 2ter. Rendre un SOUS-ÉLÉMENT de spin skippable (process réutilisable)

> **Modèle mental.** Un spin n'est PAS atomique : c'est une **suite de sous-étapes animées** que le joueur peut **skipper** (clic bouton play / Espace) pour fast-forward. Chaque jeu a des sous-étapes différentes (Space Mania : reveal des multiplicateurs de colonne → anim des soucoupes/lasers → fusion des symboles M → présentation des combinaisons gagnantes ; autre jeu : d'autres effets) MAIS le **process pour les rendre skippables est identique**. Le clic est **UN seul signal** (`stop`) qui doit skipper **la sous-étape en cours ET toutes les suivantes du spin**.

**Le signal de skip (déjà en place, ne PAS réinventer) :** le bouton play (`HudPixi.handlePlay`) et l'hotkey décident **« nouveau spin » vs « STOP »**. La branche STOP pose `stateSpin.stopRequested = true`, **broadcast `stopButtonClick`**, et **force `isTurbo`**. `stopRequested` est **remis à `false` au prochain `board-reveal`** (début du spin suivant). ⇒ un seul clic = tout le reste du spin en cours passe en mode skip.

**Process pour rendre une sous-étape X skippable (à répéter pour chaque effet) :**
1. **Poser un flag « X en cours »** en **state partagé** (`stateXxx.running` / `blocking`), mis à `true` quand X démarre (dans le handler, AVANT l'anim) et `false` à la fin (owner). Ce flag sert AUSSI de gate (le handler `await` X) — cf. §1.
2. **Ajouter ce flag à la garde « nouveau spin » de `handlePlay`** : `if (isIdle && !blockingA && !blockingB && !Xrunning) bet() else stop()`. **C'EST L'ÉTAPE LA PLUS OUBLIÉE** (§2.D) : sans ça, pendant X (qui ne pose pas forcément `isBlockingSpin`) un clic **lance un nouveau spin** au lieu de skipper — surtout en **turbo** / spins sans win/feature (la machine xstate lit déjà `idle`).
3. **Faire réagir l'anim X au skip = ACCÉLÉRER, PAS snap.** Dans le composant qui joue X, lire `stateSpin.stopRequested` et **réduire la durée** (ex. `dur = stopRequested ? FAST_MS : NORMAL_MS`). Un **snap instantané ne se voit pas** → le joueur croit que le skip ne marche pas. On veut un **fast-forward visible**.
4. **Lire `stopRequested` au DÉMARRAGE de chaque unité d'anim** (par symbole/colonne/label), pas re-cibler une anim déjà en vol. L'unité déjà en cours au moment du clic finit à sa vitesse (une seule) ; toutes les suivantes sont rapides.
5. **Le handler `await` toujours** l'anim (ne PAS sauter l'`await` conditionnellement) : c'est la **durée** qui change, pas la présence de l'anim. Ça garde la chaîne d'events déterministe et l'état (valeurs, cleanup) appliqué pareil skip ou non.

**Erreurs à NE PAS faire :**
- ❌ **Oublier le flag de X dans la garde `handlePlay`** → clic = nouveau spin au lieu de skip (le bug turbo classique).
- ❌ **Snap instantané** (résoudre la promesse / sauter l'`await`) → skip **invisible**, le joueur reclique.
- ❌ **Re-cibler un `Tween.set(...)` déjà en vol** depuis le handler `stopButtonClick` (pour accélérer l'unité en cours) → crée une **chaîne `.then` orpheline** qui se résout en retard et **clobbere** l'unité suivante (résout/nettoie la mauvaise). Préférer : durée lue au start (point 4).
- ❌ **Inventer un flag `skipX` dédié** lu par des composants enfants réactifs → race « demi-skip » (§2.A). Lire directement `stateSpin.stopRequested` (posé **synchroniquement** par `handlePlay`).
- ❌ **Zapper le cleanup** (retirer l'overlay/le number/le shrunk) dans le chemin skip : appliquer la valeur finale ET nettoyer, comme le chemin normal (réconcilier sur les valeurs autoritatives du book event à la fin).

*(Space Mania : skip du reveal count-up (`skipAppear`), du laser Vaisseau (`columnsToSkip`), et de la fusion M (`FLY_MS_SKIP` lu au start du vol) — tous derrière le MÊME `stopRequested`/`stopButtonClick`, chacun ajouté à la garde `handlePlay` (`!isBlockingSpin && !appearRunning && !fusionPending`).)*
...
| 2026-07 | HUD win | §4.6 point **1bis — HUD win = compteur incrémental synchro avec les winGroups** (pas un jump au total). La boucle `playWinSequence` possède `winBookEventAmount` et le monte par group ; l'event total (`show-final-win`/`setTotalWin`) ne le pose plus (sinon jump, aggravé par le `show-wins` fire-and-forget en base). `baseWinBook` = `0` (base/feature) ou `stateBonusWin.totalBeforeCurrentSpin` (freegame = cumul bonus). Reconcile exact (Σ groups × 100 == amount). Skip → jump au final (cf. §GATING §2ter). Généralisé depuis Space Mania. |
| 2026-07 | Gating/Skip | §GATING **§2ter — rendre un sous-élément de spin skippable** (process réutilisable + erreurs). Un spin = suite de sous-étapes animées, chacune skippable par UN clic (`stopRequested`/`stopButtonClick`). Process : (1) flag « X en cours » en state partagé (= gate) ; (2) l'ajouter à la garde `handlePlay` (sinon clic = nouveau spin, bug turbo) ; (3) skip = **ACCÉLÉRER la durée**, pas snap (invisible) ; (4) lire `stopRequested` **au start** de chaque unité, pas re-cibler un tween en vol (chaînes `.then` orphelines) ; (5) toujours `await`, seule la durée change. Généralisé depuis Space Mania (skip fusion M `FLY_MS_SKIP`, + reveal `skipAppear` + laser `columnsToSkip`, tous derrière `stopRequested`, chacun dans la garde `!isBlockingSpin && !appearRunning && !fusionPending`). |
